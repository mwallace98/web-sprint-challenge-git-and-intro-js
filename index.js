...
console.log(Object.values(artists[2]));

/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2 (not auto tested): 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
(no function needed) 
There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Fix this issue and console.log() to check your work. */

const updtartists = artists.map(obj => {
  if (obj.id == 8 ){
...
function removeArtist(array,number) {
  const copyOfArtists = array.slice();
...
function addArtist(artists,name,years,genre,nationality,bio) {
  let newArtistArray = artists.slice();
  let newArtist = {
    name: `${name}`,
    years: `${years}`,
    genre: `${genre}`,
...
console.log(addArtist(artists, 'Matt', '1998-2023','Web Design', 'Scottish', 'My Bio Here'));
...
function artistByCountry(array,string){
  ...
  if (array[i].nationality === string){
    countryArtists.push((array[i].name));
  }
}
  return countryArtists;
}
